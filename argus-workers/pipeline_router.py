...
def execute_recon_pipeline(
    ...
    slog.info(
        f"execute_recon_pipeline: target={target}, aggressiveness={aggressiveness}"
    )
...
def execute_scan_pipeline(
    ...
    slog = ScanLogger("pipeline_router")
    slog.info(
        f"execute_scan_pipeline: {len(targets)} target(s), aggressiveness={aggressiveness}, skip_tools={skip_tools}"
    )
