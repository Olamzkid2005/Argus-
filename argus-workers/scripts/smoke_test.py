...
def test_config_constants() -> bool:
    ...
    ok &= _check(
        "LLM_AGENT_MAX_COST_USD is float", isinstance(LLM_AGENT_MAX_COST_USD, float)
    )
    ok &= _check(
        "GIT_HOST_ALLOWLIST includes github.com", "github.com" in GIT_HOST_ALLOWLIST
    )
    return ok
...
def test_feature_flags() -> bool:
    ...
    ok &= _check(
        "is_enabled with no flag returns default",
        fresh_flags.is_enabled("TEST_FLAG_A", default=False) is False,
    )
    ok &= _check(
        "get_flag with no flag returns default on fresh instance",
        fresh_flags.get_flag("TEST_FLAG_B", default=42) == 42,
    )
    ok &= _check(
        "is_enabled with True default",
    ...
    ok &= _check(
        "module-level is_enabled returns default",
        is_enabled("MODULE_FLAG_A", default=False) is False,
    )
    ok &= _check(
        "module-level get_flag returns default",
...
def test_models() -> bool:
    ...
    ok &= _check("ConfidenceScorer.compute returns float 0..1", 0 <= score <= 1)
    ok &= _check(
        "ConfidenceScorer - high agreement + high evidence = high", score > 0.5
    )
...
def test_state_machine() -> bool:
    ...
    ok = True
    ok &= _check(
        "STATES include all expected states",
    ...
    ok &= _check(
        "can_transition_to('recon') is True", machine.can_transition_to("recon")
    )
    ok &= _check(
        "can_transition_to('complete') is False (from created)",
        not machine.can_transition_to("complete"),
    )
    ok &= _check(
        "get_valid_transitions returns list",
        isinstance(machine.get_valid_transitions(), list),
    )
    ok &= _check(
        "get_valid_transitions from created",
        machine.get_valid_transitions() == ["recon", "failed", "paused"],
    )

    terminal = EngagementStateMachine(eid, current_state="complete")
    ok &= _check(
        "Terminal state complete has no transitions",
    ...
    try:
        ...
        ok &= _check("Invalid state raises ValueError", "Invalid" in str(e))
...
def test_intelligence_engine() -> bool:
    ...
    # Evidence strength
    ok &= _check(
        "Verified evidence strength = 1.0",
        engine._get_evidence_strength({"evidence_strength": "VERIFIED"}) == 1.0,
    )
    ok &= _check(
        "Minimal evidence strength = 0.6",
        engine._get_evidence_strength({"evidence_strength": "MINIMAL"}) == 0.6,
    )
    ok &= _check(
        "Missing evidence defaults to 0.6", engine._get_evidence_strength({}) == 0.6
    )

    # Tool agreement
    ok &= _check(
        "Two-tool agreement = 0.85",
    ...
    ok &= _check(
        "_get_agreement_level(1) = single_tool",
        engine._get_agreement_level(1) == "single_tool",
    )
    ok &= _check(
        "_get_agreement_level(2) = medium", engine._get_agreement_level(2) == "medium"
    )
    ok &= _check(
        "_get_agreement_level(3) = high", engine._get_agreement_level(3) == "high"
    )
    ...
    ok &= _check("_group_findings_for_agreement returns dict", isinstance(groups, dict))
    ...
    ok &= _check(
        "_extract_cve_ids finds CVEs",
        "CVE-2024-12345" in cves and "CVE-2024-67890" in cves,
    )
    ok &= _check(
        "_extract_cve_ids returns empty when no matches",
        engine._extract_cve_ids({"type": "SQLI", "evidence": {}}) == [],
    )
    ok &= _check(
        "_extract_cve_ids limits to 5",
        len(
            engine._extract_cve_ids(
                {
                    "type": "",
                    "evidence": {
    ...
    ok &= _check(
        "FP detection returns dict with verdict",
        "verdict" in fp_result and "confidence" in fp_result,
    )
    ok &= _check(
        "Rich evidence yields true_positive",
...
def test_attack_graph() -> bool:
    ...
    ok &= _check(
        "add_finding adds a vuln node",
        any(n.type == "vulnerability" for n in graph.nodes.values()),
    )
    ok &= _check(
        "add_finding adds an endpoint node",
    ...
    ok &= _check(
        "get_all_paths_with_chains returns list", isinstance(paths_with_chains, list)
    )

    downstream = graph.get_downstream_paths(
        [n for n in graph.nodes if graph.nodes[n].type == "vulnerability"][0]
    )
...
def test_tool_definitions() -> bool:
    ...
    ok &= _check(
        "get_tool('nonexistent') returns None", get_tool("nonexistent_tool_xyz") is None
    )
    ok &= _check("ALL_PHASES defined", len(ALL_PHASES) >= 5)
    ok &= _check(
        "nuclei is CONFIRMED", TOOLS["nuclei"].signal_quality == SignalQuality.CONFIRMED
    )
    ok &= _check(
        "dalfox is PROBABLE", TOOLS["dalfox"].signal_quality == SignalQuality.PROBABLE
    )
    ok &= _check(
        "ffuf is CANDIDATE", TOOLS["ffuf"].signal_quality == SignalQuality.CANDIDATE
    )
...
def test_utils() -> bool:
    ...
    ok &= _check(
        "validate_uuid accepts valid UUID",
...
def test_sanitization() -> bool:
    ...
    ok &= _check(
        "sanitize_evidence escapes HTML in dict",
...
def test_error_classifier() -> bool:
    ...
    ok &= _check(
        "Connection reset -> TRANSIENT", trans.category == ErrorCategory.TRANSIENT
    )
    ...
    ok &= _check(
        "ErrorCode FILE_NOT_FOUND -> PERMANENT",
    ...
    ok &= _check(
        "tag_error attaches error_code attribute",
...
def test_cache() -> bool:
    ...
    ok &= _check(
        "cached decorator has invalidate helper",
...
def test_llm_service() -> bool:
    ...
    ok &= _check(
        "LLMService is_available() == client state",
...
def test_tracing() -> bool:
    ...
    ok &= _check(
        "get_trace_id returns None or string without context",
...
def test_cvss() -> bool:
    ...
    ok &= _check(
        "SQL_INJECTION base score = 9.8", TYPE_BASE_SCORES.get("SQL_INJECTION") == 9.8
    )
    ...
    ok &= _check("SQL_INJECTION + CRITICAL + verified = 9.8", score == 9.8)
    ok &= _check("estimate_cvss returns float 0..10", 0 <= score <= 10)

    # Test label function
    ok &= _check(
        "get_cvss_label(True) = CVSS (NVD)",
        get_cvss_label(has_cve=True) == "CVSS (NVD)",
    )
    ok &= _check(
        "get_cvss_label(False) = Estimated CVSS",
...
def test_react_agent() -> bool:
    ...
    ok &= _check(
        "can_transition_to 'analyze' from 'scan'", agent.can_transition_to("analyze")
    )
    ...
    ok &= _check(
        "invalid transition returns False", agent.transition_to("nonexistent") is False
    )
...
def test_checkpoint() -> bool:
    ...
    ok &= _check(
        "CheckpointContext has add_result",
...
def test_auth() -> bool:
    ...
    ok &= _check(
        "AuthContext not authenticated by default", ctx.is_authenticated() is False
    )
    return ok
...
def main(verbose: bool = False) -> int:
    ...
    if FAILED == 0:
        ...
        print(
            "      Review the failures above before proceeding to infrastructure tests."
        )
        return 1
