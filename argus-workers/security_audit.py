...
class SecurityAudit:
    ...
    def check_environment_variables(self):
        ...
        for var in sensitive_vars:
            ...
            if (
                value
                and any(
                    weak in value.lower() for weak in ["password", "secret", "default"]
                )
                and "localhost" not in value.lower()
            ):
    ...
    def check_file_permissions(self):
        ...
        for root, dirs, files in os.walk(_PROJECT_ROOT):
            ...
            for file in files:
                if file in sensitive_exact_names or any(
                    file.endswith(ext) for ext in sensitive_extensions
                ):
