...
_ERROR_PATTERN = re.compile(r"(error|fail|critical|vulnerability|warning)\s*[:]?\s*(.*)", re.IGNORECASE)
...
def _try_json(output: str) -> list[NormalizedFinding] | None:
    ...
    for item in items:
        if not isinstance(item, dict):
            continue
        title = (
            item.get("title") or item.get("name") or item.get("message") or item.get("finding") or "Generic finding"
        )
