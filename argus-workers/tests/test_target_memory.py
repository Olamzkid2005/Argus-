...
class TestTargetProfile:
    def test_extract_domain_full_url(self):
        assert (
            TargetProfileRepository._extract_domain("https://www.example.com/path")
            == "www.example.com"
        )

    def test_extract_domain_with_port(self):
        assert (
            TargetProfileRepository._extract_domain("example.com:8080")
            == "example.com:8080"
        )

    def test_extract_domain_no_scheme(self):
        assert TargetProfileRepository._extract_domain("example.com") == "example.com"
    ...
    def test_to_llm_context_shows_prior_scans(self):
        result = TargetProfileRepository.to_llm_context(
            None,
            {
                "total_scans": 3,
                "best_tools": [{"tool": "nuclei", "finding_count": 5}],
                "noisy_tools": ["nikto"],
