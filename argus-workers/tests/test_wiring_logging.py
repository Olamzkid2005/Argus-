...
class TestFindingVerifierLogging:
    ...
    # NOTE: This xfail hides a real bug — Orchestrator is constructed via
    # __new__() bypassing __init__, so bug_bounty_mode attribute is never set.
    # Change strict=True when the planner is fixed.
    @pytest.mark.xfail(reason="Orchestrator missing bug_bounty_mode", strict=True)
