...
class TestFireFindingWebhooks:
    ...
    def test_fires_for_critical(self):
        with patch("post_finding_hooks._get_matching_webhooks", return_value=[]) as mock_get:
            fire_finding_webhooks({
                "id": "finding-1",
                "engagement_id": "eng-001",
                "severity": "CRITICAL",
    ...
    def test_fires_webhooks(self):
        with patch("post_finding_hooks._get_matching_webhooks",
                   return_value=[{"id": "wh-1", "webhook_url": "https://hooks.example.com"}]), \
...
class TestDispatch:
    ...
    def test_successful_dispatch(self):
        ...
        with patch("post_finding_hooks.httpx.Client", return_value=FakeClient()), \
