...
def update_nuclei_templates(timeout: int = 120) -> bool:
    ...
    slog.info("Updating nuclei templates: %s", " ".join(cmd))
    logger.info("Updating nuclei templates: %s", " ".join(cmd))
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            logger.info(
                "Nuclei templates updated successfully. stdout: %s, stderr: %s",
                result.stdout[:500],
                result.stderr[:500],
            )
            return True
        else:
            slog.warn("Nuclei templates update returned code %d", result.returncode)
            logger.warning(
                "Nuclei templates update returned code %d. stdout: %s, stderr: %s",
                result.returncode,
                result.stdout[:500],
                result.stderr[:500],
            )
        ...
        slog.warn("Nuclei templates update timed out after %ds", timeout)
        logger.error("Nuclei templates update timed out after %ds", timeout)
        return False
    except FileNotFoundError:
        slog.warn("Nuclei binary '%s' not found on PATH", NUCLEI_BINARY)
        logger.warning(
        ...
        slog.warn("Nuclei templates update failed: %s", e)
