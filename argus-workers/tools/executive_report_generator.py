...
class ExecutiveReportGenerator(AbstractTool):
    ...
    def _generate_report(
        self, findings: list[dict], target: str, engagement_id: str
    ) -> dict:
        ...
        report = {
            ...
            "markdown": self._render_markdown(
                findings, target, severity_breakdown, top_findings
            ),
        }
                ...
                for f in cat_findings:
                    ...
                    if audit:
                        for rf in audit.get("risk_flags", []):
                            ...
                            if rem:
                                lines.append(f"  💡 Fix: {rem[:120]}")
                    lines.append("")

        lines.extend(
            ["---", "*Report generated by Argus Security Assessment Platform*"]
        )
        return "\n".join(lines)
