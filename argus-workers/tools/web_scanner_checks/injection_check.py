...
def _check_boolean_sqli(
    baseline: str, test_url: str, session, param: str, findings: list
):
    true_payload = "1' AND '1'='1"
    false_payload = "1' AND '1'='2"
    true_url = f"{test_url.split('?')[0]}?{param}={true_payload}"
    false_url = f"{test_url.split('?')[0]}?{param}={false_payload}"
    true_resp = safe_request(
        "GET", true_url, session, _DEFAULT_TIMEOUT, _DEFAULT_RATE_LIMIT
    )
    false_resp = safe_request(
        "GET", false_url, session, _DEFAULT_TIMEOUT, _DEFAULT_RATE_LIMIT
    )
    ...
    try:
        ...
        for param in list(params)[:5]:
            for payload in SQLI_PAYLOADS:
                ...
                test_resp = safe_request(
                    "GET", test_url, session, _DEFAULT_TIMEOUT, _DEFAULT_RATE_LIMIT
                )
                if not test_resp:
                    continue
                # Error-based detection
                if _check_error_based_sqli(
                    test_resp, findings, test_url, param, payload
                ):
                    break
        ...
        logger.warning("SQLi check failed: %s", e)
...
def _check_sqli_time_based(target_url: str, session, findings: list):
    try:
        ...
        logger.warning("SQLi time-based check failed: %s", e)
...
def _check_xss(target_url: str, session, findings: list):
    try:
        ...
        logger.warning("XSS check failed: %s", e)
...
def _check_xss_dom_based(target_url: str, session, findings: list):
    try:
        ...
        for js_url in js_urls[:5]:
            js_resp = safe_request(
                "GET", js_url, session, _DEFAULT_TIMEOUT, _DEFAULT_RATE_LIMIT
            )
        ...
        logger.warning("DOM-based XSS check failed: %s", e)
...
def _check_ssti(target_url: str, session, findings: list):
    try:
        ...
        logger.warning("SSTI check failed: %s", e)
...
def _check_lfi(target_url: str, session, findings: list):
    try:
        ...
        logger.warning("LFI check failed: %s", e)
...
def _check_xxe(target_url: str, session, findings: list):
    try:
        ...
        logger.warning("XXE check failed: %s", e)
...
def _check_cmdi(target_url: str, session, findings: list):
    try:
        ...
        logger.warning("Command injection check failed: %s", e)
