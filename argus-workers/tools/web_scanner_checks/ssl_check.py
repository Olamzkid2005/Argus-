...
def _check_hsts(target_url: str, hostname: str, session, findings: list):
    try:
        ...
        if resp is None:
            logger.debug("HSTS check: no response for %s", target_url)
            return
    except Exception:
        logger.debug("HSTS check request failed for %s", target_url)
        return
...
def _check_cert_expiry(cert: dict, hostname: str, port: int, findings: list):
    ...
    try:
        ...
        logger.debug(
            "Could not parse notAfter date for %s:%s: %s", hostname, port, not_after
        )
        return
