...
class DualAuthScanner(AbstractTool):
    ...
    def for_phase_execution(
        ...
        *,
        ...
        # CRITICAL: set _builder so findings route through FindingBuilder.
    ...
    def execute(self, ctx: ToolContext) -> UnifiedToolResult:
        ...
        if session_a:
            ...
            if not owned_resources:
                ...
                for bopla in self._check_bopla(session_a, "user_a"):
                    self._builder.add(
                        bopla["type"], bopla["severity"], bopla["endpoint"],
                        bopla["evidence"], confidence=bopla["confidence"],
                    )
            else:
                ...
                if session_b:
                    ...
                    for bf in bola_findings:
                        self._builder.add(
                            bf["type"], bf["severity"], bf["endpoint"],
                            bf["evidence"], confidence=bf["confidence"],
                        )
        ...
        # Sync findings back to self.findings for backward compat with callers
        # that read scanner.findings directly (e.g. regression tests).
