        ...
        try:
            ...
            for item in items:
                ...
                if any(
                    ...
                    for kw in ["token", "auth", "key", "secret", "password"]
                ):
                    severity = "HIGH"
        ...
        for line in raw_output.split("\n"):
            ...
            if " - " in line:
                ...
                if any(x in description.lower() for x in ["xss", "sql", "injection"]):
                    ...
                    for x in ["default", "credential", "password"]
                ):
                    severity = "CRITICAL"
        ...
        for line in raw_output.split("\n"):
            ...
            try:
                ...
                if technologies:
                    ...
                    severity = "INFO"
                    if any(
                        t.lower() in [s.lower() for s in sensitive]
            ...
            for leak in data:
                finding = {
                    ...
                    "evidence": {
                        ...
                        "line": leak.get(
                            "StartLine", leak.get("startLine", leak.get("line", 0))
                        ),
                        "secret_type": leak.get(
                            "RuleID",
                            leak.get("ruleId", leak.get("Description", "unknown")),
                        ),
