...
import { Effect, Exit, Cause, Layer, Option, Context, Schema, Stream } from "effect"
  ...
  Effect.gen(function* () {
    ...
    const finishAuth = Effect.fn("MCP.finishAuth")(function* (mcpName: string, authorizationCode: string) {
      ...
      if (Exit.isFailure(exit)) {
        ...
        const errorMsg = failReason ? String(failReason.error) : "OAuth completion failed"
        pendingOAuthTransports.delete(mcpName)
        return { status: "failed", error: errorMsg } as Status
      }
