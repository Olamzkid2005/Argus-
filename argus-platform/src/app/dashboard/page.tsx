...
import { EngagementProgress } from "@/components/ui/EngagementProgress";
...
export default function DashboardPage() {
  ...
  const {
    ...
    onEvent: (_event: WebSocketEvent) => {
      // Handle event - no logging in production
      void _event; // acknowledge param
      // Update current state from events
      if (_event.type === "state_transition") {
        setCurrentState(_event.data.to_state as string);
      }
    },
    onError: (err: Error) => {
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      setApproveError(
        err instanceof Error ? err.message : "Failed to approve engagement",
      );
    } finally {
  ...
  // Get engagement ID from URL params on mount
  useEffect(() => {
      ...
      <div className="max-w-7xl mx-auto flex flex-col gap-8">
        ...
        <div className="prism-glass p-8 rounded-3xl flex flex-col md:flex-row gap-6 items-center justify-between">
          <div className="flex flex-col gap-1">
            ...
            <div className="flex items-center gap-2 text-sm text-muted-foreground font-medium">
              <div
                className={`w-2 h-2 rounded-full ${wsConnected ? "bg-argus-cyan" : "bg-red-500"} animate-pulse`}
              />
        ...
        {isConnected && currentState && (
          <div className="prism-glass p-6 rounded-2xl">
            <EngagementProgress currentState={currentState} />
          </div>
        )}
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {[
            {
              label: "Detected Vectors",
              val: eventsByType["finding_discovered"]?.length || 0,
              color: "text-argus-magenta",
            },
            {
              label: "Operational Loops",
              val: eventsByType["state_transition"]?.length || 0,
              color: "text-argus-indigo",
            },
            {
              label: "Tool Executions",
              val: eventsByType["tool_executed"]?.length || 0,
              color: "text-argus-cyan",
            },
            {
            ...
            {events.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-muted-foreground gap-4 opacity-50">
                ...
                <p className="text-sm font-medium">
                  Standalone system waiting for signal...
                </p>
              </div>
            ) : (
              <AnimatePresence initial={false}>
                {events.map((event, index) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`mt-0.5 p-2 rounded-lg ${meta.bg} ${meta.color}`}
                      >
        ...
        <div className="text-sm">
          <p>
            ...
            <span className="text-white">
              {event.data.finding_type as string}
            </span>
          </p>
          ...
          <p className="text-slate-500 text-xs mt-1">
            ...
            | Tool: {event.data.source_tool as string}
          </p>
        </div>
      );
        ...
        <div className="text-sm">
          <p>
            <span className="text-slate-400">
              {event.data.from_state as string}
            </span>
            <span className="mx-2">→</span>
            <span className="text-blue-300">
              {event.data.to_state as string}
            </span>
          </p>
    ...
    case "rate_limit_event":
      return (
        <div className="text-sm">
          <p>
            Domain:{" "}
            <span className="text-white">{event.data.domain as string}</span>
          </p>
          <p className="text-slate-400 text-xs mt-1">
            {event.data.message as string} | RPS:{" "}
            {(event.data.current_rps as number).toFixed(1)}
          </p>
        </div>
      );
      ...
      return (
        <div className="text-sm">
          <p>
            ...
            {" | "}
            <span
              className={event.data.success ? "text-green-400" : "text-red-400"}
            >
              {event.data.success ? "Success" : "Failed"}
            </span>
          </p>
          <p className="text-slate-500 text-xs mt-1">
            Duration: {event.data.duration_ms as number}ms | Findings:{" "}
            {event.data.findings_count as number}
          </p>
        </div>
      );
